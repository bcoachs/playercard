...
setProject(res.item||null)).catch(()=>setProject(null))
  ...
  function InputsForSelected(){
    ...
    const n = st.name.toLowerCase()
    const displayIdx = ST_INDEX[st.name] ?? '?'
    const heading = `S${displayIdx} - ${st.name}`.toUpperCase()
      ...
      return (
        <div className="capture-panel__form-card">
          ...
          <div className="capture-panel__form-actions">
            <button className="btn" type="button" onClick={() => saveOne(st, player)}>
              Speichern
            </button>
            {renderPdfButton()}
          </div>
